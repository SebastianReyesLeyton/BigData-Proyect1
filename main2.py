...
class Project1:
    ...
    def __init__(self, result='result_2.md'):
    ...
    def datasetTransformation(self):
        ...
        self.df = self.df.filter((self.df['nombre_dx'] == 'DIARREA Y GASTROENTERITIS DE PRESUNTO ORIGEN INFECCIOSO') |
        ...
        file = str(self.results)
        for col in uniques:
            self.results = f'{RESULTS_TRANSFORM}{col}_2.md'
            self.cleanFile()
            elements = list(map(str, uniques[col]))
            self.storeResults({ 'subsubsubtitle': col, 'enumeration': elements })
        ...
        fig.savefig(f'{GRAPHICS}correlation_2.png', bbox_inches='tight')
